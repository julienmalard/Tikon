...
class PruebaExperParc(unittest.TestCase):
    ...
    def test_grupo_parcelas(símismo):
        exp = Exper(
            "mi experimento", parcelas=GrupoParcelas([Parcela("una"), Parcela("otra")])
        )
        símismo.assertEqual(len(exp.parcelas), 2)
    ...
    def test_grupo_y_parcelas(símismo):
        exp = Exper(
            "mi experimento",
            parcelas=[Parcela("una"), GrupoParcelas([Parcela("y"), Parcela("otra")])],
        )
    ...
    def test_parecelas_idénticas(símismo):
        with símismo.assertRaises(ValueError):
            Exper(
                ...
                parcelas=[
                    Parcela("una"),
                    GrupoParcelas([Parcela("y"), Parcela("una")]),
                ],
            )
    ...
    def test_controles(símismo):
        ...
        for cntrl in ["superficies", "elevaciones", "centroides", "distancias"]:
            with símismo.subTest(cntrl):
                símismo.assertListEqual(
                    exp.controles[cntrl].coords[EJE_PARC], ["una", "otra"]
                )
