...
class PruebaModelo(unittest.TestCase):
    ...
    def test_módulo_con_punto(símismo):
        ...
        with símismo.assertRaises(ErrorNombreInválido):
            módulo_con_punto.modelo.simular(
                "módulo con punto", exper=módulo_con_punto.exper, t=10
            )

    def test_var_con_punto(símismo):
        with símismo.assertRaises(ErrorNombreInválido):
            var_con_punto.modelo.simular(
                "var con punto", exper=var_con_punto.exper, t=10
            )
    ...
    def test_req_modelo_falta(símismo):
        ...
        with símismo.assertRaises(ErrorRequísitos):
            req_modelo_falta.modelo.simular(
                "simul modelo falta", exper=req_modelo_falta.exper, t=10
            )
    ...
    def test_req_var_falta(símismo):
        ...
        with símismo.assertRaises(ErrorRequísitos):
            req_var_falta.modelo.simular(
                "var modelo falta", exper=req_var_falta.exper, t=10
            )
    ...
    def test_req_control_falta(símismo):
        ...
        with símismo.assertRaises(ErrorRequísitos):
            req_cntrl_falta.modelo.simular(
                "simul modelo falta", exper=req_cntrl_falta.exper, t=10
            )
    ...
    def test_poner_valor(símismo):
        ...
        with símismo.subTest(relativo=False):
            ...
            npt.assert_equal(
                res["exper"]["módulo"]["res"].res[{EJE_TIEMPO: slice(1, None)}].values,
                1,
            )
        with símismo.subTest(relativo=True):
            res = modelo_rel.simular(
                "valor control", exper=exper, t=10, vars_interés=True
            )
            ...
            xrt.assert_equal(
                ...
                xr.DataArray(
                    np.arange(11),
                    coords={EJE_TIEMPO: datos_res[EJE_TIEMPO]},
                    dims=[EJE_TIEMPO],
    ...
    def test_obt_valor_extern():
        ...
        xrt.assert_equal(
            res["exper"]["módulo 1"]["res 1"].res, res["exper"]["módulo 2"]["res 2"].res
        )
    ...
    def test_res_inicializable():
        modelo, exper, const = (
            res_inicializable.modelo,
            res_inicializable.exper,
            res_inicializable.const,
        )
    ...
    def test_múltiples_exper(símismo):
        modelo = poner_valor.modelo
        expers = [
            Exper("exper", Parcela("parcela")),
            Exper("exper 2", Parcela("parcela")),
        ]
    ...
    def test_reps_ent():
        ...
        exper = obt_valor.exper
        exper.controles["var"] = 2
        res_mód = modelo.simular("valor control", exper=exper, t=2, reps=4)["exper"][
            "módulo"
        ]["res 1"]
...
class PruebaTiempo(unittest.TestCase):
    ...
    def test_tiempo_de_obs():
        ...
        npt.assert_equal(
            res.res[EJE_TIEMPO].values, pd.date_range(f_inic, f_final, freq="D")
        )
    ...
    def test_tiempo_numérico_de_obs():
        ...
        res = modelo.simular("tiempo numérico", exper=exper, t=10)["exper"]["módulo"][
            "res"
        ]
        npt.assert_equal(
            res.res[EJE_TIEMPO].values, pd.date_range(f_inic, periods=11, freq="D")
        )
    ...
    def test_obs_tiempo_numérico():
        ...
        npt.assert_equal(
            res.res[EJE_TIEMPO].values, pd.date_range(f_inic, periods=5, freq="D")
        )
    ...
    def test_tiempo_fecha_de_obs_tiempo_numérico():
        ...
        modelo = tiempo_obs.modelo
        f_inic = "2000-01-01"
        res = modelo.simular("obs numérico", exper=exper, t=f_inic)["exper"]["módulo"][
            "res"
        ]
        npt.assert_equal(
            res.res[EJE_TIEMPO].values, pd.date_range(f_inic, periods=5, freq="D")
    ...
    def test_tiempo_num_de_obs_tiempo_numérico():
        exper = tiempo_obs.exper_obs_numérico
        modelo = tiempo_obs.modelo
        res = modelo.simular("obs numérico", exper=exper, t=10)["exper"]["módulo"][
            "res"
        ]
        npt.assert_equal(
            res.res[EJE_TIEMPO].values,
            pd.date_range(date.today(), periods=11, freq="D"),
        )
    ...
    def test_no_obs_no_tiempo():
        ...
        modelo = tiempo_obs.modelo
        res = modelo.simular("sin tiempo o obs", exper, vars_interés=True)["exper"][
            "módulo"
        ]["res"]
        npt.assert_equal(
            res.res[EJE_TIEMPO].values,
            pd.date_range(date.today(), periods=31, freq="D"),
        )
