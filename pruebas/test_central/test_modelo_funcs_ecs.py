...
class PruebaFuncionalidadesEcs(unittest.TestCase):
    def _verif_en(símismo, res, rango):
        if isinstance(res, Resultado):
            res = res.datos
        símismo.assertTrue(
            np.all(np.logical_and(rango[0] <= res.matr, res.matr <= rango[1]))
        )
    ...
    def _verif_fuera_de(símismo, res, rango):
        if isinstance(res, Resultado):
            res = res.datos
        símismo.assertTrue(
            np.any(np.logical_or(rango[0] > res.matr, res.matr > rango[1]))
        )
    ...
    def test_postproc():
        ...
        res_pp_categ = res.datos.loc[
            res.datos.codificar_coords({"coso": coso_pp_categ})
        ].matr
        ...
        npt.assert_equal(
            res_pp,
            1 + ec_postproc.agreg_postproc_categ + ec_postproc.agreg_postproc_subcateg,
        )
    ...
    def test_obt_y_poner_valor():
        ...
        res = modelo.simular("ec obt valor", exper, t=10, vars_interés=True)["exper"][
            "módulo"
        ]
    ...
    def test_obt_valor_control():
        ...
        res = modelo.simular("ec obt valor control", exper, t=2)["exper"]["módulo"][
            "res"
        ]
    ...
    def test_obt_valor_extern():
        ...
        exper.controles["var control"] = valor
        res = modelo.simular("ec obt valor extern", exper, t=2)["exper"]["módulo"][
            "res"
        ]
        npt.assert_equal(res.datos.matr, valor)
    ...
    def test_poner_valor_extern():
        ...
        modelo, exper, valor = (
            ec_poner_valor_extern.mi_modelo,
            ec_poner_valor_extern.mi_exper,
            ec_poner_valor_extern.valor,
        )
        exper.controles["var control"] = valor
        res = modelo.simular("ec obt valor extern", exper, t=2)["exper"]["otro módulo"][
            "res 2"
        ]
        npt.assert_equal(res.datos.matr, valor)
    ...
    def test_apriori_auto(símismo):
        ...
        modelo, exper, rango = (
            ec_apriori_auto.mi_modelo,
            ec_apriori_auto.mi_exper,
            ec_apriori_auto.rango,
        )
    ...
    def test_apriori_coso(símismo):
        ...
        coso.espec_apriori(
            apriori=APrioriDist(uniform(3, 1)),
            categ="categ",
            sub_categ="subcateg",
            ec="ec",
            prm="a",
    ...
    def test_inter(símismo):
        ...
        with símismo.subTest("sin interacciones"):
            res = modelo.simular(
                "sin interacciones", exper=exper, t=2, vars_interés=True
            )["exper"]["módulo"]["res"]
        ...
        with símismo.subTest("con interacciones"):
            ...
            res = modelo.simular(
                "con interacciones", exper=exper, t=2, vars_interés=True
            )["exper"]["módulo"]["res"]
            símismo._verif_en(
                res.datos.loc[
                    res.datos.codificar_coords({"coso": coso1, "otro": [coso2, coso3]})
                ],
                rango,
            )
    ...
    def test_inter_aprioris(símismo):
        ...
        with símismo.subTest("con índices"):
            ...
            símismo._verif_en(
                res.datos.loc[
                    res.datos.codificar_coords({"coso": coso1, "otro": coso3})
                ],
                (1.5, 1.5 + 1),
            )
            símismo._verif_fuera_de(
                res.datos.loc[
                    res.datos.codificar_coords({"coso": coso1, "otro": coso2})
                ],
                (1.5, 1.5 + 1),
            )
