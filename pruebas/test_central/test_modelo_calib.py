...
class PruebaCalibEcs(unittest.TestCase):
    ...
    def test_calib_paráms_mód(símismo):
        ...
        dists_disp_1 = coso1.ecs["categ"]["subcateg"]["ec"]["a"].dists_disp(
            inter=None, heredar=False
        )
        dists_disp_2 = coso2.ecs["categ"]["subcateg"]["ec"]["a"].dists_disp(
            inter=None, heredar=False
        )
...
class PruebaCalibInic(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.valid = modelo.simular(
            "valid", exper, calibs=EspecCalibsCorrida(["calib"])
        ).validar()
    ...
    def test_cargar_calib(símismo):
        ...
        valid = otro_modelo.simular(
            "valid", otro_exper, calibs=EspecCalibsCorrida(["calib"])
        ).validar()
    ...
    def test_renombrar_calib(símismo):
        ...
        valid = modelo.simular(
            "valid", exper, calibs=EspecCalibsCorrida(["otro nombre"])
        ).validar()
        símismo.assertAlmostEqual(valid["ens"], símismo.valid["ens"], delta=0.05)
