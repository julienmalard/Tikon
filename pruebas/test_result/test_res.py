...
class PruebaResultado(unittest.TestCase):
    def test_iter_índs(símismo):
        ...
        datos = xarray.DataArray(0.0, coords=coords, dims=["eje1", "eje2", "eje3"])
        ...
        with símismo.subTest("excluir"):
            ...
            símismo.assertEqual(
                len(índs),
                np.prod([len(v) for ll, v in coords.items() if ll != excluir]),
            )
            símismo.assertTrue(
                all(eje in índ for índ in índs for eje in coords if eje != excluir)
            )
