...
class PruebaRed(unittest.TestCase):
    def test_1_insecto(símismo):
        res = Modelo(r.red_1_insecto).simular(
            "1 insecto", exper=r.exper, t=10, depurar=True
        )

    def test_depred(símismo):
        símismo.assertSetEqual(r.red_depred.presas(), set(r.sencillo.etapas))
        res = Modelo(r.red_depred).simular(
            "depred sub", exper=r.exper, t=10, depurar=True
        )

    def test_depred_mútliples(símismo):
        símismo.assertSetEqual(
            set(r.red_depred_mútliples.presas()),
            set(r.sencillo.etapas).union(r.otro_sencillo.etapas),
        )
        res = Modelo(r.red_depred_mútliples).simular(
            "depred", exper=r.exper, t=10, depurar=True
        )

    def test_parasitismo(símismo):
        res = Modelo(r.red_parasitismo).simular(
            "parasitismo", exper=r.exper, t=10, depurar=True
        )

    def test_esfécido(símismo):
        res = Modelo(r.red_esfécido).simular(
            "esfécido", exper=r.exper, t=10, depurar=True
        )

    def test_hiperparasitismo(símismo):
        res = Modelo(r.red_hiperparasitismo).simular(
            "esfécido", exper=r.exper, t=10, depurar=True
        )
    ...
    def test_parasitoide_generalista(símismo):
        ...
        res = Modelo(r.red_paras_generalista).simular(
            "paras generalista", exper=r.exper, t=10, depurar=True
        )
