...
class PruebaExterno(unittest.TestCase):
    ...
    def _prueba_parc(símismo, parc):
        res = Modelo([símismo.mód_cult, símismo.red]).simular(
            "cultivo", exper=Exper("exper", parc), t=5, vars_interés=True
        )["exper"]

        npt.assert_equal(res[Cultivo.nombre][RES_BIOMASA].res.values, símismo.biomasa)
        npt.assert_equal(
            res[Cultivo.nombre][RES_HUMSUELO].res.values, símismo.hum_suelo
        )
        npt.assert_equal(
            res[RedAE.nombre][RES_POBS]
            .res.loc[{EJE_ETAPA: símismo.tomate.etapas}]
            .values,
            símismo.biomasa,
        )
    ...
    def test_combin_estoc(símismo):
        símismo._prueba_parc(
            MiParcelaCultivoExterno(
                Parcela("parc"), símismo.hum_suelo, símismo.biomasa, combin="estoc"
            )
        )
...
class PruebaCombinSimsCult(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.coords = {
            "eje 1": ["a", "b", "c"],
            **{ll: range(v) for ll, v in cls.reps.items()},
        }
        cls.datos = Datos(0.0, dims=list(cls.coords), coords=cls.coords)

    def _verificar_agregar(símismo, combin, coords):
        agregado = combin.agregar(símismo.datos)
        xrt.assert_equal(
            agregado.a_xarray(), xr.DataArray(0.0, coords=coords, dims=list(coords))
        )
        return agregado
    ...
    def _verificar_desagregar(símismo, combin, agregado):
        ...
        xrt.assert_equal(
            desagregado.a_xarray(),
            símismo.datos.transponer(desagregado.dims).a_xarray(),
        )

    def test_transf_lista(símismo):
        combin = CombinSimsCult(("estoc", "paráms"))
        agr = símismo._verificar_agregar(
            combin, coords={"eje 1": símismo.coords["eje 1"]}
        )
    ...
    def test_transf_texto(símismo):
        ...
        agr = símismo._verificar_agregar(
            combin, coords={dim: símismo.coords[dim] for dim in ["eje 1", "paráms"]}
        )
        símismo._verificar_desagregar(combin, agr)
