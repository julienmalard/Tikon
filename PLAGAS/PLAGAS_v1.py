...
class Insecto(object):
    ...
    def __init__(self,Nombre,depred=[],pres=[]):    #depred, prens = listas
        ...
        self.depred = depred
        self.pres = pres
#        self.ecuación = ""                          #Crear en la calibración
...
def gen_ecua_dep(insecto, presas):  #insecto depredadores (con presas)
    ...
    for p in presas:
        ...
        sub_ecuación = sub_ecuación + "+ %(b['"+ p +"']) * ((%(C['"+ p +"'])/poblaciones['"+ insecto +"']^m *"\
            "poblcaiones['"+ p +"'])/(1 + %(C['"+ p +"'])/poblaciones['"+ insecto +"']^m*%(h['"+ p +"'])*poblaciones['"+ p +"']))"\
            "*poblaciones['"+ p +"']/'"+ pres_adición +"'"
        ecuación = "poblaciones['"+ insecto + "'] * ("+ sub_ecuación +")" #Añadirlo a la ecuación
    return ecuación
...
def ad_ecua_dep(insecto,depredadores):
    ...
    for d in depredadores:
        ecuación = ecuación + "- poblaciones['"+ d +"']*((%(C['"+ d +"'])/poblaciones['"+ d +"']^m * poblaciones['"+ insecto +"'])/(1 + %(C['"+ d +"'])"\
        "/poblaciones['"+ d +"']*%(h['"+ d +"'])*poblaciones['"+ insecto +"']"
    return ecuación
    ...
    def __init__(self,nombre,insectos,ecuaciones={}): 
        ...
        if len(self.ecuaciones):            #Si e modelo ya tiene ecuaciones calibradas... (tiene que tener TODAS las ecuaciónes de los insectos del modelo)
            ...
            for i in self.insectos:         #Para cada insecto de interés...
                self.ecuaciones[i] = {"Ecuación":{}, "Coef":{}}       # (le quité: eval(i).ecuación)
...
###Pruebas
Araña = Insecto("Araña",pres=["Mosca"])
Mosca = Insecto("Mosca",depred=["Araña"])
Ensayo = mod_plagas("Ensayo",["Mosca","Araña"])
