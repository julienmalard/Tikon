...
git_setup() {
  ...
  git config --global user.name "Robot de Transifex (Travis CI)"
}
...
tx_push() {
  ...
  if [ $? -eq 0 ] && [ "$TRAVIS_BRANCH" = "$TX_BRANCH" ]; then
    tx_init
    update_translations
...
tx_pull() {
  ...
  echo $TRAVIS_JOB_NUMBER | grep "\.1$"
  if [ $? -eq 0 ] && [ "$TRAVIS_TAG" = "$TX_TAG" ]; then
    echo "pulling"
    tx_init
    tx pull --all --force
    FRESH_TRANSLATIONS=$(git diff-index --name-only HEAD --)
    if [ -n "$FRESH_TRANSLATIONS" ]; then
      echo "pushing"
      git_setup
      commit_translation_files
      push_translation_files
