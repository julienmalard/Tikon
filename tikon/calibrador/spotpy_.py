...
class CalibSpotPy(Calibrador):
    ...
    def _calc_calib(símismo, func, dists, n_iter):
        ...
        with tempfile.NamedTemporaryFile(
            "w", encoding="UTF-8", prefix="calibTiko'n_"
        ) as d:
            args = dict(
                spot_setup=mod_spotpy,
                dbname=d.name,
                parallel="seq",
                dbformat="csv",
                save_sim=False,
...
class ModSpotPy(object):
    ...
    def parameters(símismo):
        return spt.parameter.generate(
            [
                _gen_var_spotpy(d.dist, "var_" + str(í))
                for í, d in enumerate(símismo.dists)
            ]
        )
...
def _gen_var_spotpy(dist, nmbr_var):
    ...
    if nombre_dist == "Chi2":
        ...
        var = spt.parameter.Triangular(
            nmbr_var, left=ubic, mode=(prms[0] * escl) + ubic, right=ubic + escl
        )
    else:
