...
class ResultadoRed(Resultado):
    ...
    def __init__(símismo, sim, coords, vars_interés):
        if (
            EJE_ETAPA not in coords
        ):  # Hay que verificar porque `Cohortes` lo implementa sí mismo
            coords = {
                EJE_ETAPA: [x for x in sim.ecs.cosos_en_categ(símismo.nombre)],
                **coords,
            }
    ...
    def cerrar(símismo):
        for eje in símismo.ejes_etps:
            fantasmas = [
                e for e in símismo._datos_t.coords[eje] if isinstance(e, EtapaFantasma)
            ]
            paras_juv = [
                e
                for e in símismo._datos_t.coords[eje]
                if isinstance(e, EtapaJuvenilParasitoide)
            ]
            if paras_juv:
    ...
    def apriori_de_obs(símismo, obs, índ):
        ...
        def _extr_ref(e=None):
            if e:
                índ_e = {
                    EJE_ETAPA: e,
                    **{ll: v for ll, v in índ.items() if ll != EJE_ETAPA},
                }
            else:
                índ_e = índ
            datos_obs = [
                o_.datos.loc[índ_e].dropna(EJE_TIEMPO)[{EJE_TIEMPO: 0}]
                for o_ in obs
                if índ_e in o_
            ]
            return (
                (
                    np.mean(datos_obs),
                    max(np.std(datos_obs), np.mean(datos_obs) * 0.15, 1),
                )
                if datos_obs
                else None
            )
        ...
        if ref:
            ...
            return APrioriDens((mín, ref[0] + ref[1]), 0.80)
        if isinstance(etp, EtapaFantasma):
            ...
            if ref_espejo:
                ref_espejo = (
                    ref_espejo[0] / len(co_huéspedes),
                    ref_espejo[1] / len(co_huéspedes),
                )
            ...
            if ref_mín:
                ...
                return APrioriDens((mín, ref_mín[0] + ref_mín[1]), 0.80)
