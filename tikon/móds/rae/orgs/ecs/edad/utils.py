...
def días_grados(mín, máx, umbrales, método, corte):
    ...
    if método == "triangular":
        ...
        sup_centro = donde(
            dif > 0,
            (1 - máximo((umbr_máx - mín) / (máx - mín), 0)) * (umbr_máx - umbr_mín),
            0,
        )
        ...
        sup_lados = donde(
            ...
            * donde(
            ...
            + máximo(mín - umbr_mín, 0)
            * donde(dif > 0, 1 - (umbr_máx - mín) / (máx - mín), 1),
            0,
        )
        ...
        intr_máx = donde(
            f_numpy(np.logical_or, umbr_máx_nrm < -1, umbr_máx_nrm > 1),
            1,
            -umbr_máx_nrm,
        ).fi(np.arccos)
        ...
        intr_mín = donde(
            f_numpy(np.logical_or, umbr_mín_nrm < -1, umbr_mín_nrm > 1),
            1,
            -umbr_mín_nrm,
        ).fi(np.arccos)
        ...
        sup_centro = (
            donde(dif > 0, (i_máx[1] - i_máx[0]) * (umbr_máx_nrm - umbr_mín_nrm), 0)
            / (2 * π)
            * amp
        )
