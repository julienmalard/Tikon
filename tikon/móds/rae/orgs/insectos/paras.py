...
class Parasitoide(Insecto):
    ...
    def parasita(
        ...
        if etp_emerg is None:
            siguiente = (
                etps_entra if isinstance(etps_entra, Etapa) else etps_entra[-1]
            ).siguiente()
            etp_emerg = siguiente or huésped.etapas[-1]

        super().parasita(
            huésped=huésped,
            etp_símismo=etp_símismo,
    ...
    def juveniles_en(
        símismo, huésped: ResolvableAEtapas, incluir_parasitadas=True
    ) -> EspecificaciónEtapas:
        ...
        tipo_ec = {
            ...
            "juvenil": {
                ...
                ECS_MOV: {
                    "Distancia": "Nada",
                    "Atracción": "Nada",
                },  # Juveniles de esfécidos no se mueven
            },
        }
...
class EspecificaciónEtapasParasitadas(EspecificaciónEtapas):
    ...
    def concuerda(símismo, etapa: Etapa) -> bool:
        if isinstance(etapa, EtapaFantasma):
            if etapa.org is símismo.parasitoide and símismo.huésped.concuerda(
                etapa.etp_hués
            ):
                return True
        return False
