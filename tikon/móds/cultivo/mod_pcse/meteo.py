...
class ProveedorMeteoPCSEPandas(NASAPowerWeatherDataProvider):
    ...
    def __init__(símismo, bd_pandas, lat, lon, elev):
        ...
        if "veloc_viento" not in bd_pandas:
            bd_pandas["veloc_viento"] = (
                1  # en m/s; valor automático consistente con DSSAT
            )
        ...
        bd_pandas = bd_pandas.rename(
            {vr: vr_pcse for vr, vr_pcse in conv_nombres.items() if vr in bd_pandas},
            axis="columns",
        )
        ...
        for f in bd_pandas.index:
            ...
            fila = {
                "DAY": día,
                **{
                    conv_nombres[vr] if vr in conv_nombres else vr: bd_pandas[vr][f]
                    for vr in bd_pandas.columns
                },
            }
            ...
            e0, es0, et0 = reference_ET(
                LAT=lat,
                ELEV=elev,
                ANGSTA=símismo.angstA,
                ANGSTB=símismo.angstB,
                ETMODEL="PM",
                **fila,
            )
