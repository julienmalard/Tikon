...
class Modelo(object):
    def __init__(símismo, módulos):
        if isinstance(módulos, Módulo) or (
            isclass(módulos) and issubclass(módulos, Módulo)
        ):
            módulos = [módulos]
    ...
    def _filtar_paráms(símismo, paráms, en):
        ...
        if isinstance(en, (str, Módulo, Coso, Parám, Exper)):
            en = [en]
        en = [
            next(m for nmbr, m in símismo.módulos.items() if nmbr == x)
            if isinstance(x, str)
            else x
            for x in en
        ]
        return [
            ...
            if any(
                prm.prm.coso == x
                if isinstance(x, Coso)
                else prm.prm == x
                if isinstance(x, Parám)
                else prm.prm.coso == x
                if isinstance(x, Exper)
...
def _gen_reps(reps, calib=False, paráms=None):
    ...
    if extras:
        raise ValueError(
            "{extras} no es valor aceptable para eje de repetición.".format(
                extras=", ".join(extras)
            )
        )
    base.update(reps)
