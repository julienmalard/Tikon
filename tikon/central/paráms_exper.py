...
class ParámsExperVar(PlantillaParámsExper):
    def __init__(símismo, nombre, exper, sim, datos, n_reps):
        ...
        if símismo.datos.obs:
            ref = min(
                líms[1],
                np.nanmax([np.nanmax(o_.datos.values) for o_ in símismo.datos.obs]),
            )
            if líms == (0, np.inf):
        ...
        índs = list(
            sim.iter_índs(datos=sim.datos, excluir=[EJE_TIEMPO, EJE_PARÁMS, EJE_ESTOC])
        )
        coords = {
            dim: sim.datos.coords[dim]
            for dim in sim.datos.dims
            if dim not in [EJE_TIEMPO, EJE_PARÁMS, EJE_ESTOC]
        }
        ...
        def _gen_matr_prm(crds, índs_=None, inter=None):
            ...
            if crds:
                ...
                return cls(
                    [
                        _gen_matr_prm(crds=crds, índs_=í, inter=inter + [í])
                        for í in índs_
                    ],
                    eje=eje,
                    índice=inter[-1] if len(inter) else None,
                )
            else:
                return ValsParámCoso(
                    tmñ=símismo.n_reps, prm_base=símismo.prm, índice=índs_, inter=inter
                )
    ...
    def __init__(símismo, nombre, exper, sim, datos, n_reps):
        ...
        for var in símismo.sim:
            if símismo.sim[var].inicializable:
                ...
                símismo[var] = ParámsExperVar(
                    var, exper, símismo.sim[var], datos=datos, n_reps=n_reps
                )
...
class ParámsExper(PlantillaParámsExper):
    def __init__(símismo, exper, sim):
        super().__init__(
            exper.nombre, exper, sim, datos=exper.datos, n_reps=sim.reps["paráms"]
        )
        ...
        for mód in símismo.sim:
            ...
            símismo[mód] = ParámsExperMód(
                mód, exper, símismo.sim[mód], datos=datos, n_reps=símismo.n_reps
            )
