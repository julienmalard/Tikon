...
def _sacar_vals_paráms(red):
    dic_egr = {}
    for n_etp, d_etp in enumerate(red.etapas):
        etp = d_etp['nombre']
        org = d_etp['org']
            ...
            for s_categ, d_s_categ in d_categ.items():
                for tipo_ec, d_t_ec in d_s_categ.items():
                    for parám, val_p in d_t_ec.items():
                        d_val['ubic_parám'] = [categ, s_categ, tipo_ec, parám]
                        n_rel_etp = red.ecs[categ][s_categ][tipo_ec].index(n_etp)
                        if len(val_p.shape) == 2:  # Si no hay interacciones...
                            d_val['val'] = val_p[0, n_rel_etp]
                            dic_egr[org].append(d_val)
                        else:
                            for i in range(val_p.shape[2]):
                                val = val_p[0, n_rel_etp, i]
                                if not np.isnan(val):
                                    d_val_inter = d_val.copy()
                                    d_val_inter['org_inter'] = red.etapas[i]['nombre']
                                    d_val_inter['etp_inter'] = red.etapas[i]['org']
                                    d_val_inter['val'] = val
...
def _gen_a_prioris(vals, prec):
    ...
    for org, l_a_pr in vals.items():
        ...
        for d_v in l_a_pr:
            ...
            val = dic.pop('val')
            dic['rango'] = (max(rango[0], val - rango[0] * (100 - prec) / 100),
                            min(rango[1], val + rango * (100 - prec) / 100))
            dic['certidumbre'] = 1
...
def _simul_a_exp(red):
    ...
    etps_interés = [etp for i, etp in enumerate(red.etapas) if all(i not in f.values() for f in red.fantasmas)]

    archivo = 'temp.csv'

    with open(archivo, 'w', newline='') as arch_csv:
        escritorcsv = csv.writer(arch_csv)
        escritorcsv.writerow(['Etapa %i' % i for i in etps_interés])
        for día in range(preds.shape[-1]):
            l_vals = list(preds[0,0,0,etps_interés,día])
            escritorcsv.writerow(l_vals)

    exper = Experimento(nombre='Simulado', proyecto=Red_coco.proyecto)
...
Red_coco.validar(nombre='Valid con verdaderos', exper=Exper_artificial)

# Intentar calibrar, y validar, con rango de menos en menos restringidos
for p in range(100, 90, -1):
    a_pr = _gen_a_prioris(vals=vals_paráms, prec=p)
    _aplicar_a_prioris(red=Red_coco, d_a_pr=a_pr)
    Red_coco.calibrar(nombre='Calib con prec. {}'.format(p), aprioris=a_pr, quema=0, n_iter=10)
    valid = Red_coco.validar(nombre='Valid con calib prec. {}'.format(p), a_prioris=a_pr)
    pprint(valid)
