...
def graficar_res(
    ...
    etiq_y = (
        "{var} ({unids})".format(var=simulado.name, unids=simulado.attrs["unids"])
        if etiq_y is None
        else etiq_y
    )
    ...
    if incert is None:
        ...
        ejes.fill_between(
            ...
            facecolor=color,
            alpha=0.3,
            label="Incert estocástico",
        )

    else:
        raise ValueError(
            '`incert` debe ser "componentes" o "confianza", no  "%s".' % incert
        )
