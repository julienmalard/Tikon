...
class Files(DocDssat):
    def __init__(self, dic={}):
        self.dic = dic
        if len(self.dic) == 0:
            self.dic = {"ID_SOIL": [], "SLSOURCE": [], "SLTX": [], "SLDP": [], "SLDESCRIP": [],
    ...
    def escribir(self,cod_suelo):
        ...
        with open("C:\DSSAT45\Soil\Python.sol", "r+") as d:
            doc = d.readlines()
            print(len(doc))
            for i,línea in enumerate(doc):
                if cod_suelo in línea:
...
class Filec(DocDssat):
    def __init__(self, nombre, cultivo, modelo, dic={}):
        self.dic = dic
        self.nombre = nombre
        self.cultivo = cultivo
        self.modelo = modelo
        if len(self.dic) == 0:
            self.dic = {"VAR#": "", "VRNAME": self.nombre[0:14], "ECO#": "", "CSDL": [], "PPSEN": [], "EM-FL": [],
                        "FL-SH": [], "FL-SD": [],
            ...
            self.dic["VAR#"] = "TIK" + self.nombre[0:2]
        # La lista de los documentos de genética de cultivos disponibles para DSSAT
        self.doc_cult_disp = {"MZCER046": {"Maíz": "MZCER046"},
